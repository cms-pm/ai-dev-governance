# Claude CodeGraph Adapter

**Source of truth:** `core/CODE_INTELLIGENCE_GOVERNANCE.md`.

## Purpose

Defines Claude-specific conventions for using CodeGraph (CG) as the optional
Tier-2 source-code intelligence surface in strict ADG consumers. This adapter
maps the core Code Intelligence Governance doctrine into `CLAUDE.md`,
`.mcp.json`, and Claude settings fragments without redefining the policy.
...
Claude consumers that enable CG should apply the template fragments under
`templates/codegraph/`:
...
from the template. In consumers that declare CG, Claude sessions MUST perform
lightweight CG lookups for source navigation before broad `Glob`, `Grep`,
`Read`, recursive listing, or multi-file read passes whose purpose is
discovery.
...
- `core/EVIDENCE_CONTRACT.md`
- `adapters/providers/CLAUDE_CONTEXT_ADAPTER.md`
- `templates/codegraph/CLAUDE.md.fragment`
- `templates/codegraph/.mcp.json.fragment`
- `templates/codegraph/settings.json.fragment`
