...
|---|---|---|---|---|---|---|---|
| R-10-01 | Code-intelligence bounded-context glossary ambiguity — Astaire vs CodeGraph vs Graphify scope drift in downstream consumers | Low | Medium | SCN-10.1, SCN-10.6 | New `core/CODE_INTELLIGENCE_GOVERNANCE.md` lands the three-tier doctrine and the bounded-context glossary entries under the `DOMAIN_LANGUAGE_GOVERNANCE.md` pattern (per-context authoring authority + naming-correspondence rule). SCN-10.6 demotes graphify and amends `.graphifyignore` to deny code directories — explicit handoff to CG. Closure at SCN-10.10 once doctrine ratified. | Accountable Delivery Lead | At SCN-10.1, SCN-10.6, SCN-10.10 |
...
| R-10-03 | Graphify second-order effects — agents that previously discovered nothing in graphify now fall back to glob/grep without CG (consumer not yet on CG) | Medium | Medium | First consumer not yet on CG; SCN-10.6 demotion landing | The graphify demotion in SCN-10.6 explicitly documents the handoff. Plan B (LSDF) is documented at `templates/codegraph/PLAN_B_LSDF.md` for Python-only / no-Docker consumers. The Tier-3 scope narrowing applies regardless of whether the consumer has adopted Tier-2 yet — graphify simply returns to its research-corpus role; glob/grep/Read are the documented fallback per `CLAUDE.md.fragment`. | Accountable Delivery Lead | At SCN-10.6 review; first consumer adoption window |
...
- **SCN-10.6 rollback.** Revert the graphify skill scope narrowing and
  the `.graphifyignore` code-dir denies. Graphify returns to its
  pre-SCN-10.6 trigger documentation; code dirs are no longer denied
  (graphify still produces files, agents still don't read them — no
  behavioural change at the consumer).
