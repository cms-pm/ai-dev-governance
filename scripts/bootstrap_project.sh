...
write_codegraph_contract() {
  local path="${CONSUMER_ROOT}/docs/governance/codegraph-contract.md"
  local template="${GOVERNANCE_REPO_ROOT}/templates/CODEGRAPH_CONTRACT_TEMPLATE.md"
  if [[ -f "$path" ]]; then
    return 0
  fi
  if [[ "$MODE" == "new" ]] || [[ "$FORCE" == true ]]; then
    ...
    cp "$template" "$path"
    info "Created docs/governance/codegraph-contract.md"
  else
    check "Would create: docs/governance/codegraph-contract.md"
    DRIFT_FOUND=true
  fi
}

codegraph_state() {
  local manifest="${CONSUMER_ROOT}/governance.yaml"
  if [[ ! -f "$manifest" ]]; then
    echo NO
    return 0
  fi
  ...
  grep -q "codegraphIndexFreshnessURI:" "$manifest" && has_freshness=true
  grep -q "codegraphImageDigestURI:" "$manifest" && has_digest=true
  if [[ "$has_freshness" == true && "$has_digest" == true ]]; then
    echo DECLARED
  elif [[ "$has_freshness" == true || "$has_digest" == true ]]; then
    echo PARTIAL
  else
    echo NO
  fi
...
| CodeGraph contract doc present | $([[ -f "${CONSUMER_ROOT}/docs/governance/codegraph-contract.md" ]] && echo PASS || echo FAIL) | $(date -u +"%H:%M:%SZ") |
| CodeGraph declaration state | $(codegraph_state) | $(date -u +"%H:%M:%SZ") |
...
5. Review \`docs/governance/codegraph-contract.md\`; leave CG undeclared or
...
if [[ "$MODE" == "new" ]]; then
  ...
  if [[ -x ".astaire/astaire" ]]; then
    ...
    .astaire/astaire startup --root . 2>&1 | grep -v "^INFO" || true
  fi
