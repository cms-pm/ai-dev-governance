...
|--------------------|------------------------|--------------------|---------------|
| v1.2.3             | strict                 | No (removes the bundled vendor `.claude/skills/` tree and repins the bundled Astaire to v0.6.2, its first mainline tag since v1.0.0; no ADG doctrine change. Requires one `astaire startup`/`init` after taking the pin to migrate the widened `document_fts` table before any `scan`/`sync`/`register` - see CHANGELOG `[1.2.3]` Upgrade steps. Consumers relying on the Astaire claim/entity/relationship/contradiction tables being created by a default `astaire init` must now pass `--with-claims`, as that subsystem became optional in Astaire v0.6.0) | `astaire` @ `v0.6.2` (`505a63b`) |
| v1.1.5             | strict                 | No (patch over v1.1.4 - allows top-level `raw/` to be indexed by default and adds CodeGraph lock-contention wait/retry for rapid agent-driven calls) | `astaire` @ `v0.5.0` (`ed16f6d`) |
| v1.1.4             | strict                 | No (patch over v1.1.3 - fixes declared-CG empty indexing by staging sanitized `/workspace`, parses JSON/CLI/MCP status counts, and uses live status instead of host `.codegraph/` mtime for Docker named-volume freshness) | `astaire` @ `v0.5.0` (`ed16f6d`) |
